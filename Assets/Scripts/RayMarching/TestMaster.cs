...
public class TestMaster : MonoBehaviour
{
    ...
    public void OnRandomizeCPU()
    {
        for (int i = 0; i < repetitions; i++)
        {
            for (int c = 0; c < objects.Count; c++)
            {
                ...
                obj.transform.position = new Vector3(obj.transform.position.x, obj.transform.position.y, Random.Range(0.0f, 1f));
    ...
    private void OnGUI()
    {
        ...
        {
            if (GUI.Button(new Rect(0, 100, 200, 100), "Random CPU"))
            {
                OnRandomizeCPU();
            }
            if (GUI.Button(new Rect(0, 0, 200, 100), "Random GPU"))
            {
                OnRandomizeGPU();
            }
        }
    ...
    //private void OnRenderImage(RenderTexture sre, RenderTexture dest)
    //{
    ...
    //    computeShader.Dispatch(0, renderTexture.width / 10, renderTexture.height / 1, 1);
    //
